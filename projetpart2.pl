...
# my @tmp2;
# my $insert2=$dbh->prepare("INSERT into General(Entry, EntryName, Status, GeneName, ProteinName, EnsemblProt)VALUES(?,?,?,?,?,?)");
...
#   chomp;
#   @tmp2= split(/\t/,$_);
#   $insert2->execute($tmp2[0],$tmp2[1],$tmp2[2],$tmp2[4],$tmp2[3],$tmp2[9]);
