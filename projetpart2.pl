...
# $dbh->do("create table General(Entry varchar primary key, EntryName varchar, Status varchar, GeneName varchar, EnsemblProt varchar)");
...
# my @tmp2;
# my $insert2=$dbh->prepare("INSERT into General(Entry, EntryName, Status, GeneName, EnsemblProt)VALUES(?,?,?,?,?)");
...
#   chomp;
#   @tmp2= split(/\t/,$_);
#   $insert2->execute($tmp2[0],$tmp2[1],$tmp2[2],$tmp2[4],$tmp2[9]);
